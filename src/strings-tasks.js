...
function formatTime(minutes, seconds) {
  const hoursStr = String(minutes).padStart(2, '0');
  const minutesStr = String(seconds).padStart(2, '0');
...
function isPalindrome(str) {
  const alphaStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  return value.sort((a, b) => b.length - a.length)[0];
}
...
function extractEmails(str) {
  const value = str.replaceAll(';', ' ').split(' ');
  return value;
}
  ...
  return str
    ...
    .map((char) => {
      ...
      if (charCode >= 65 && charCode <= 90) {
        value = String.fromCharCode(((charCode - 65 + 13) % 26) + 65);
      } else if (charCode >= 97 && charCode <= 122) {
        value = String.fromCharCode(((charCode - 97 + 13) % 26) + 97);
      }
      return value;
    })
    .join('');
